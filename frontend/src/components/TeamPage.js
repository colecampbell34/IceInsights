  ...
  if (loading)
    return (
      <div className="min-h-screen flex justify-center items-center bg-gray-100">
        <p className="text-xl text-gray-600">Loading roster...</p>
      </div>
    );
  if (error)
    return (
      <div className="min-h-screen flex justify-center items-center bg-gray-100">
        <p className="text-xl text-red-600">Error loading roster data. Please try again later.</p>
      </div>
    );

  return (
    <div className={`${teamColor} min-h-screen flex flex-col items-center`}>
      <header className={`w-full py-12 text-white text-center ${teamColor} shadow-md`}>
      ...
      <section className="w-full px-6 py-8 bg-opacity-80 bg-gray-800 shadow-lg rounded-lg max-w-7xl -mt-12">
        {roster.length === 0 ? (
          <p className="text-center text-gray-500 text-lg">No players available for this team.</p>
        ) : (
            ...
            {roster.map((player) => (
              ...
              >
                <div className="flex flex-col items-center">
                  ...
                  <p className="text-lg text-gray-600">{player.positionCode}</p>
                  <p className="text-lg text-gray-500">#{player.sweaterNumber}</p>
                </div>
              </Link>
            ))}
          </div>
